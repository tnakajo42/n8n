...
以下をClaude Codeにそのまま貼り付けてください。
`{{VARIABLES}}` の部分だけ事前に書き換えてから実行してください。
...
| 変数 | 説明 | 例 |
|------|------|----|
| `{{GCP_PROJECT_ID}}` | GCPプロジェクトID | `intertextured-488406` |
| `{{GCP_ACCOUNT}}` | gcloud認証済みアカウント | `t.nakajo@drivex.co.jp` |
